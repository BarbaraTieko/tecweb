...
Vamos criar um teste para a função `divide()` que verifica se ela lança uma exceção `ZeroDivisionError` quando tentamos dividir por zero:

```python
import pytest

def test_divide_por_zero_gera_ZeroDivisionError():
...
Nesse teste, utilizamos o bloco `with pytest.raises(ZeroDivisionError):` para verificar se a função `divide()` lança uma exceção do tipo `ZeroDivisionError`. Se a exceção for lançada, o teste passa. Caso contrário, ele falha.
