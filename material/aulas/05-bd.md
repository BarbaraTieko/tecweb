...
!!! tip "Usuários Windows"
    Talvez seja necessário algumas configurações extras para o sistema operacional Windows.

    Caso o docker mencione a utilização do WSL siga os passos em: [Instalando WSL](https://github.com/InsperGuides/Guia-Instalacao-WSL-e-Docker---Windows)

Faça o download da imagem do Postgres (é a definição de um sistema com o Postgres instalado):

    docker pull postgres

Crie uma pasta no seu computador para armazenar os dados do banco de dados (você pode criar em outro diretório se preferir). Se o seu sistema operacional for windows, rode este comando via terminal do PowerShell.
...
Agora, utilize os comandos a seguir para criar o banco de dados e o usuário que serão utilizados pela sua aplicação (algumas configurações a seguir são recomendadas pela [documentação do Django](https://docs.djangoproject.com/en/4.1/ref/databases/#optimizing-postgresql-s-configuration)):
...
!!! danger "Importante"
    Sempre que você for rodar o seu projeto django que está utilizando o postgres, será necessário ligar o container com o comando utilizando no começo do handout:

        docker run --rm --name pg-docker -e POSTGRES_PASSWORD=escolhaumasenha -d -p 5432:5432 -v $HOME/docker/volumes/postgres:/var/lib/postgresql/data postgres

## pgAdmin

Caso queira visualizar as tabelas criadas no banco de dados, será necessário instalar o [pgAdmin](https://www.pgadmin.org)
