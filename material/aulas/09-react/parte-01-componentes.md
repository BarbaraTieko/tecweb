...
Vamos começar fazendo alterações para começarmos a refazer o Projeto 1B utilizando Django REST + React.
...
Em nosso projeto, podemos penser em quebrar a aplicação em componentes menores. Por exemplo, podemos criar um componente para representar uma `card`, `formulario`, `appbar` e assim por diante.
...
Crie também o arquivo `src/components/Note/index.css`  e adicione as classes CSS `.card`, `.card-title` e `.card-content` que você implementou no Desafio CSS:
