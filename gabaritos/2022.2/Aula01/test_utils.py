...
class LoadDataTestCase(unittest.TestCase):
    def test_load_data_from_file(self):
        ...
        verifica_caminho1 = Path(m.call_args[0][0]) == Path('data') / 'data.json'
        verifica_caminho2  = Path(m.call_args[0][0]) == CUR_DIR / 'data' / 'data.json'
        
        msg = f'A função deve procurar o arquivo dentro da pasta templates. a função está procurando o arquivo em {Path(m.call_args[0][0])}'
        self.assertTrue(verifica_caminho1 or verifica_caminho2, msg=msg)
...
class LoadTemplateTestCase(unittest.TestCase):
    def assert_template_loaded(self, filename, expected):
        ...
        verifica_caminho1 = Path(m.call_args[0][0]) == Path('templates') / filename
        verifica_caminho2  = Path(m.call_args[0][0]) == CUR_DIR / 'templates' / filename
